...
from rectovaldigittools import RectByExtentTool, RectFromCenterTool, SquareFromCenterTool, CircleFromCenterTool, \
    OvalByExtentTool, OvalFromCenterTool, RotateTool
...
class RectOvalDigit:
    ...
    def initGui(self):
        ...
        # Add actions
        self.rectbyextent = QAction(QIcon(":/plugins/rectovalDigit/icons/rectbyextent.png"), "Rectangle by extent",
                                    self.iface.mainWindow())
        self.rectfromcenter = QAction(QIcon(":/plugins/rectovalDigit/icons/rectfromcenter.png"),
                                      "Rectangle from center", self.iface.mainWindow())
        self.squarefromcenter = QAction(QIcon(":/plugins/rectovalDigit/icons/squarefromcenter.png"),
                                        "Square from center", self.iface.mainWindow())
        self.circlefromcenter = QAction(QIcon(":/plugins/rectovalDigit/icons/circlefromcenter.png"),
                                        "Circle from center", self.iface.mainWindow())
        self.ovalbyextent = QAction(QIcon(":/plugins/rectovalDigit/icons/ovalbyextent.png"), "Ellipse by extent",
                                    self.iface.mainWindow())
        self.ovalfromcenter = QAction(QIcon(":/plugins/rectovalDigit/icons/ovalfromcenter.png"), "Ellipse from center",
                                      self.iface.mainWindow())

        self.toolBar.addActions(
            [self.rectbyextent, self.rectfromcenter, self.squarefromcenter, self.circlefromcenter, self.ovalbyextent,
             self.ovalfromcenter])
        ...
        # Add rotate
        self.rotaterectoval = QAction(QIcon(":/plugins/rectovalDigit/icons/rotate.png"), "Rotate rectangle or oval",
                                      self.iface.mainWindow())
    ...
    def createFeature(self, geom):
        ...
        QgsMessageLog.logMessage(settings.value("/qgis/digitizing/disable_enter_attribute_values_dialog"), 'General')
        for i in range(fields.count()):
            f.setAttribute(i, provider.defaultValue(i))

        # if not (settings.value("/qgis/digitizing/disable_enter_attribute_values_dialog")):
        '''
        ^ commented out because this condition will not work if the QGIS user has fiddled with
        the 'reuse last entered attribute values' settings in QGIS options. For whatever reason,
        this permanently messed with the disable_enter_attributes_value_dialog and the boolean value
        will be different for users depending on their workspace. Now, the attributes dialog box will
        open regardless of the QGIS settings. 
